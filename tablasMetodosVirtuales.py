...
def nuevaSubClase(lista):
    ...
    if superclase in tipos:
        if len(metodos) == len(
                set(metodos)
        ):  #Si fuesen posibles los ciclos en la herencia aqui se verificarian
...
def buscarMetodos(superclase):
    ...
    while continua:
        ...
        contenido = list(tipos[superclase])
        contenido.pop(0)
        u = [x for x in contenido if x not in metodosReEscritos]
        metodosReEscritos+= u
        tiposDisponibles.append((superclase, u))
        if herencia == None:
            continua = False
        else:
            superclase = herencia
    ...
    for y in range(0, len(metHerencia)):
        ...
        for x in range(0, len(metodos)):
            if metodos[x] in metClase:
                ...
                print(metodos[x] + " -> " + metHerencia[y][0] + " :: " +
                      metodos[x])
